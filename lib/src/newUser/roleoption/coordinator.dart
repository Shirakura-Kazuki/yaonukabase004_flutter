...
class _CoordinatorState extends State<Coordinator> {
  ...
  List<String> menuItems1 = ['未選択']; //主メニュー
  List<String> menuItems2 = ['未選択']; 

  List<String> _currentTechOptions = ['未選択']; //選択された項目による動的なプルダウンメニューリスト
  ...
  Future<void> _fetchTechOptions() async {
    try {
      final response = await http.get(Uri.parse('https://getfielddisplaynames.azurewebsites.net/api/get_field_display_name'));
      if (response.statusCode == 201) {
        final Map<String, dynamic> responseData = json.decode(response.body);
        final List<dynamic> data = responseData['data'];
        setState(() {
          menuItems1 = ['未選択'] + data.map<String>((item) => item.toString()).toList();
          _tech1 = menuItems1[0]; // 初期値を設定
        });
        print('取得した値は、$menuItems1'); // 取得したデータをコンソールに表示
      } else {
        print('Failed to fetch tech options');
      }
    } catch (e) {
      print('Error fetching tech options: $e');
    }
  }

  Future<void> _fetchTechOptionsa() async {
    try {
      final response = await http.get(Uri.parse('https://getskilldisplaynames.azurewebsites.net/api/get_skill_display_name?'));
      if (response.statusCode == 201) {
        final Map<String, dynamic> responseData = json.decode(response.body);
        final List<dynamic> data = responseData['data'];
        setState(() {
          menuItems2 = ['未選択'] + data.map<String>((item) => item.toString()).toList();
          _tech2 = menuItems2[0]; // 初期値を設定
          _currentTechOptions = menuItems2; // 初期値を設定
        });
        print('取得した値は、$menuItems2'); // 取得したデータをコンソールに表示
      } else {
        print('Failed to fetch tech options');
      }
    } catch (e) {
...
//         });
        ...
        var requestBody = json.encode({
          ...
          'userprofile': _profile,
          'numberofpoints': 0,
          'coordinatorskillname': _tech2,
          'coordinatorfieldname': _tech1,
        });
                ...
                child: Container(
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      buildDropdown(_tech1, '自分が詳しい分野', menuItems1, (String? newValue) {
                        setState(() {
                          _tech1 = newValue!;
                        });
                      }),
                      const SizedBox(height: 16),
                      buildDropdown(_tech2, '自分が詳しい技術', menuItems2, (String? newValue) {
                        setState(() {
                          _tech2 = newValue!;
                        });
                      }),
...
// //   //   // ここでtech1に応じた動的なプルダウンメニューを設定
// //   //   // APIから取得したメニューを使う場合は必要に応じて変更する
// //   //   setState(() {
// //   //     _currentTechOptions = ['未選択'];
// //   //     _tech2 = '未選択';
// //   //   });
// //   // }
...
// //   Future<void> _fetchTechOptionsa() async {
// //     try {
// //       final response = await http.get(Uri.parse('https://getfielddisplaynames.azurewebsites.net/api/get_skill_display_name'));
// //       if (response.statusCode == 201) {
// //         final Map<String, dynamic> responseData = json.decode(response.body);
// //         final List<dynamic> data = responseData['data'];
// //         setState(() {
...
// //                       }),
// //                       const SizedBox(height: 16),
// //                       buildDropdown(_tech2, '自分が詳しい技術', _currentTechOptions, (String? newValue) {
// //                         setState(() {
// //                           _tech2 = newValue!;
// //                         });
// //                       }),
