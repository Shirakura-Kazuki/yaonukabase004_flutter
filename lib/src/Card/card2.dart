...
class _HorizontalCardListsState extends State<HorizontalCardLists> {
  ...
  String userId = '';
  late List<int> Concepts;
  late List<int> tecn1;
  late List<int> tecn2;
  late List<int> tecn3;
  late List<int> ideaDates;
  ...
  void initState() {
    ...
    Concepts = List.generate(widget.titles.length, (index) => 0);
    tecn1 = List.generate(widget.titles.length, (index) => 0);
    tecn2 = List.generate(widget.titles.length, (index) => 0);
    tecn3 = List.generate(widget.titles.length, (index) => 0);
    ideaDates = List.generate(widget.titles.length, (index) => 0);
    ...
    return Container(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          return buildCard(widget.titles[index], cardWidth, index, widget.colorNumbers[index], widget.ideaId[index], widget.likeNum[index] , widget.Concepts[index], widget.tec1[index],widget.tec2[index],widget.tec3[index],widget.ideaDate[index]);
        },
      ),
    );
  }

  Widget buildCard(String title, double width, int index, int colorNumber, int ideaId, int likeNum ,String concept,String tec1,String tec2,String tec3,String ideaDate,) {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => IdeaDataSpecial(
              ...
              // ideaconcept: '高精度の自動車部品を鋳造するためのシステム', // 例として固定値を使用
              ideaconcept: widget.Concepts[index], // 例として固定値を使用
              ideatechnology1: widget.tec1[index], // 例として固定値を使用
              ideatechnology2: widget.tec2[index], // 例としてnullを使用
              ideatechnology3: widget.tec3[index], // 例としてnullを使用
              ideadate: widget.ideaDate[index], // 例として固定値を使用
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Text(_truncateTitle(title), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
